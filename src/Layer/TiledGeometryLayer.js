...
class TiledGeometryLayer extends GeometryLayer {
    ...
    update(context, layer, node) {
        ...
        if (node.visible) {
            ...
            if ((node.pendingSubdivision || (TiledGeometryLayer.hasEnoughTexturesToSubdivide(context, node) && this.subdivision(context, this, node))) && this.isExceedSourceZoomMax(context, node)) {
            ...
            if (node.material.visible) {
                if (!requestChildrenUpdate) {
                    if (context.view.enableMaterialUpdateOpt) {
                        for (const attachedLayer of this.attachedLayers) {
                            if (attachedLayer.ready && attachedLayer.isColorLayer) {
                                attachedLayer.update(context, attachedLayer, node, node.parent);
                            }
                        }
                        return node.materialLevelNeedsUpdate ? undefined : ObjectRemovalHelper.removeChildren(this, node);
                    }
    ...
    isExceedSourceZoomMax(context, node) {
        const elevationLayerMaxZooms = context.elevationLayers.map(e => e.source.zoom.max);
        const colorLayerMaxZooms = context.colorLayers.map(e => e.source.zoom.max);
        const zoomMax = Math.max(...elevationLayerMaxZooms, ...colorLayerMaxZooms);
        return node.level < zoomMax;
    }
